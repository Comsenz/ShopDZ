...
(function() {
    ...
    var plugins = /^(https?:\/\/.+)\/plugins/i.exec(location.href);
	if(plugins && plugins.length > 1){
			SiteUrl = plugins[1] + '/index.php';
			ApiUrl = plugins[1] + '/api.php';
			WapSiteUrl = plugins[1] + '/wap';
			LogoUrl = plugins[1] + '/data/Attach/';
			rootUrl = plugins[1]+'/';
	}else{
		if (m && m.length > 1) {
...
$(function(){
	...
	if($('#header').length){
		...
		header +='<a href="javascript:;" class="toggle-icon"><img src="'+WapSiteUrl+'/img/toggle.png" class="toggle-img"/></a>';
		header +='<a href="/wap"><img src="'+WapSiteUrl+'/img/logo.png" class="logo"/></a>';
		header +='<a href="basket.html" class="head-cartimg"><img src="'+WapSiteUrl+'/img/cart.png" class="shop-cart-img"/><span class="shop-cart-have" style="display: block;">' + goodsnum + '</span></a>';
		header+='<img src="'+WapSiteUrl+'/img/toggle.png" class="img-none"/>'
		header +='</div>';
		header +='</div>';
		$('#header').html(header);
	}
	...
	if($('#header_has_back').length){
			...
			header_has_back +='<img src="'+WapSiteUrl+'/img/back.png" class="back-icon"/>';
			header_has_back +='<h1 class="head-tit" id="head_tit" >页面标题</h1>';
			header_has_back +='<a href="'+WapSiteUrl+'/basket.html" class="head-cartimg"><img src="'+WapSiteUrl+'/img/cart.png" class="shop-cart-img"/><span class="shop-cart-have" style="display: block;">' + goodsnum + '</span></a>';
			header_has_back +='</div>';
			header_has_back +='</div>';
	...
	if($('#header_has_nav').length){
			...
			header_has_nav +='<img src="'+WapSiteUrl+'/img/back.png" class="back-icon"/>';
...
/*	if($('#header_has_nav_card').length){
	...
	if($('#plugins_header_has_nav').length){
			var header_has_nav = '';
			header_has_nav +='<div class="header-box">';
			header_has_nav +='<div class="index-header goback-header">';
			//header_has_nav +='<a href="javascript:;" class="toggle-icon"><img src="img/toggle.png" class="toggle-img"/></a>';
			header_has_nav +='<a href="'+WapSiteUrl+'">';
			header_has_nav += ('<img src="'+WapSiteUrl+'/img/logo.png" class="logo"/></a>');
			header_has_nav +='<a href="javascript:;" class="head-cartimg"><span class="head-more head-moreimg head-moreimgred"></span></a>';
			header_has_nav +='</div>';
			header_has_nav +='</div>';
			header_has_nav += '<ul class="nav-list radius3" style="display:none;">';
			...
			header_has_nav += '<li><a href="'+WapSiteUrl+'/news.html" class="navnews">消息<span class="basket-have">' + goodsnum + '</span></a></li>';
			header_has_nav += '</ul>';
			$('#plugins_header_has_nav').html(header_has_nav);
	}
	if($('#member_header_has_nav').length){
			var header_has_nav = '';
			header_has_nav +='<div class="header-box">';
			header_has_nav +='<div class="index-header goback-header">';
			header_has_nav +='<a href="javascript:;" class="toggle-icon"><img src="'+WapSiteUrl+'/img/toggle.png" class="toggle-img"/></a>';
			header_has_nav += '<a href="'+WapSiteUrl+'"><img src="'+WapSiteUrl+'"/img/logo.png" class="logo"/></a>';
			header_has_nav +='<a href="javascript:;" class="head-cartimg"><span class="head-more head-moreimg head-moreimgred"></span></a>';
			header_has_nav +='</div>';
			header_has_nav +='</div>';
			header_has_nav += '<ul class="nav-list radius3" style="display:none;">';
...
//		});
	...
	if($('#footer').length){
		var  footer=  '<div class="foot1">'
						+'<p class="footbtn-box"><a href="'+WapSiteUrl+'">商城首页</a><a href="'+WapSiteUrl+'/member.html">个人中心</a></p>'
	//+'<p class="icp"><span>网站版权所有</span><a href="http://www.miibeian.gov.cn" target="_blank">豫ICP备000001号</a></p>'
						+'<p class="icp"><span><a href="http://www.miibeian.gov.cn" target="_blank">ICP证</span></a>&nbsp;<a href="javascript:;" id="footernumber"></a></p>'
    					+'<p class="icp"><span id="footer_info">ShopDZ版权所有&nbsp;©&nbsp;</span><a href="http://www.shopdz.cn" target="_blank"></a></p>'
    					+'</div>'
...
function initPage(){
	...
	if(bodywidth<540){ 
		...
		$("body").find(".fixedBox").css({"width":"540px","left":(bodywidth/2-270)+'px'});
