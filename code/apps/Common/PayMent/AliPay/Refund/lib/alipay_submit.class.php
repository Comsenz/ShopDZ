...
class AlipaySubmit {
     ...
     * @param $jump_url   提交表单之后跳转的链接
     * @return 提交表单HTML文本
     */
	function buildRequestForm($para_temp, $method, $button_name,$jump_url='') {
		//待请求参数数组
		$para = $this->buildRequestPara($para_temp);
		$sHtml = "<meta charset='UTF-8' ><form target='_blank'  style='display:none;'  id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_gateway_new."' method='".$method."'>";
		while (list ($key, $val) = each ($para)) {
            $sHtml.= "<input type='hidden' name='".$key."' value='".$val."'/>";
        }
		//submit按钮控件请不要含有name属性
        $sHtml = $sHtml."<input type='button'  id='sub'   value='".$button_name."'></form>";
		$sHtml = $sHtml.'<script>
