...
use Common\PayMent\WxPay\JsApiPay;
use Common\PayMent\WxPay\WxPayDataBase;
use Common\PayMent\WxPay\WxPayUnifiedOrder;
use Common\PayMent\WxPay\WxPayNativePay;
...
class WxPay {
	...
	public function MakeSign($data)
	{
		...
		$string = trim($string, "&");
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".get_wxpay_config('key');
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
	...
	function JsApi(){
		...
		$input->SetNotify_url(C('PAYMENT_URL'));
