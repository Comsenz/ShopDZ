<div class="tipsbox radius3">
	...
	<ol class="tips-list" id="tips-list">
		<li>1.商品分类将会对应到前台的商品分类列表里，编辑新商品时可修改编辑到某分类下。</li>
...
<div class="iframeMain">
    <ul class="transverse-nav">
        <li class="activeFour"><a href="javascript:;"><span>编辑商品分类</span></a></li>
    </ul>
	<div class="white-bg">
	    <form  method="post"  autocomplte="off"  action="<?php echo U('Commodity/category_edit');?>"   method="post"  id="main_form"  >  
		<input type="hidden" name="gc_id" value="<?php echo $category_info['gc_id'] ?>"/>
		<div class="tab-conbox">
			<div class="jurisdiction boxsizing">
				<dl class="juris-dl boxsizing">
					<dt class="left text-r boxsizing"><span class="redstar">*</span>分类名称：</dt>
					<dd class="left text-l">
					    <input type="text" name="gc_name"   value="<?php echo $category_info['gc_name']; ?>"  localrequired="localrequired"  class="com-inp1 radius3 boxsizing" />
					...
					<dd class="left text-l">    
						<div class=" boxsizing radius3 inline borderR-none">
    						<select   class="left addFocus-sele"  name="gc_parent_id" id="gc_parent_id">、
    							<?php if ($category_info['gc_parent_id'] == 0){ ?>
    								<option value="0" selected>顶级分类</option>
    							<?php } else { ?>
	    							<?php if(!empty($parent)){  ?>
	                                    <?php foreach($parent as $one){ ?>
	                                    <option value="<?php echo $one['gc_id'];?>" <?php if($category_info['gc_parent_id'] == $one['gc_id']){ ?> selected <?php } ?>><?php echo $one['gc_name'];?></option>
	                                    <?php } ?>
	                                <?php }  ?>
	                            <?php } ?>
    						</select>
				...
				<dl class="juris-dl boxsizing">
					...
					<dd class="left text-l">    
						<input type="text" name="desc"  value="<?php echo $category_info['desc']; ?>"   class="com-inp1 radius3 boxsizing" />
				...
				<dl class="juris-dl boxsizing">
                    ...
                    <dd class="left text-l">
                        <div class="switch-box">
                            <input type="checkbox" name="is_show" id="switch-radio" class="switch-radio"  value="1" <?php if($category_info['is_show']){  echo  'checked="checked"'; } ?> />
                ...
                <dl class="juris-dl boxsizing">
					...
					<dd class="left text-l">    
						<input type="text" name="listorder"   value="<?php echo $category_info['listorder']; ?>" class="com-inp1 radius3 boxsizing" />
						<p class="remind1">请输入排序值，商品分类将按照这个值降序排列</p>
					</dd>
				</dl>
				 <dl id="gc_image_dl" class="juris-dl boxsizing  <?php if($category_info['gc_parent_id']==0){ echo 'none';}?>">
			            ...
			            <ul class="uploadbox boxsizing">
							<li class="left uploadbox-li boxsizing">
								<div class="img-style boxsizing">
									<img  shopdz-action="upload_image" src="__ATTACH_HOST__/<?php echo $category_info['gc_image']; ?>" alt="" class="uploadimg boxsizing"/>
								</div>
								<i  shopdz-action="upload_delete"  class="up-icon dele-icon"></i>
								<div class="operationbox boxsizing">
									<p class="upload-p">
									    <input shopdz-action="upload_value"  value="<?php echo $category_info['gc_image']; ?>"  type="hidden"  name="gc_image"  >
			...
			<div class="btnbox3 boxsizing submit_div <?php if($category_info['gc_parent_id']==0){ echo 'borderT-none';}?>">
...
$(function(){
	...
	$('.uploadbox>li').mouseleave(function(){
       ...
       $('.icon-cover').bind('click',function(){
       		$('.icon-cover').hide();
            $('.icon-alert').hide();
       })
