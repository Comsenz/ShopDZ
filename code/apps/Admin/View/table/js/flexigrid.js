...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ /* 应用默认的属性 */
            ...
            __mw: { /* 可扩展的中间件功能对象 */
                datacol: function(p, col, val) { /* 中间件格式化数据列 */
                    var _col = (typeof p.datacol[col] == 'function') ? p.datacol[col](val) : val; /* 格式列使用函数 */
                    if(typeof p.datacol['*'] == 'function') { /* 如果通配符函数存在 */
                        return p.datacol['*'](_col); /* 动行通配符函数 */
                    } else {
                        return _col; /* 返回没有通配符的纵列 */
                    }
                }
            },
			...
			addData: function (data) { /* 解析数据 */
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row) {
						...
						$('thead tr:first th', g.hDiv).each( /* 添加单元格监视 */
							function () {
								...
								if($(this).attr('checkall')){
									/* 多选框操作 */
									td.innerHTML = '<input type="checkbox" id="check-'+row.cell[p.colModel[idx].id]+'" class="regular-radio" value="'+row.cell[p.colModel[idx].id]+'"/><label for="check-'+row.cell[p.colModel[idx].id]+'"></label>';
								}else if($(this).attr('handle')){
									/* 按纽操作 */
									if(typeof(p.colModel[idx].button) == 'function')
										td.innerHTML = p.colModel[idx].button(row.cell[idx]);
								}else{
									// 如果每一行是对象本身(没有监视) */
									if (typeof row.cell == 'undefined') {
	                                    ...
	                                    td.innerHTML = p.__mw.datacol(p, $(this).attr('abbr'), iHTML); /* 使用中间件datacol格式关口 */
									}
									/* 如果内容有一个<背景= nnnnnn >选项,解码。 */
