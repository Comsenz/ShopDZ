 ...
 *
 *      $Id: install_function.php 2016/8/10 15:00  seeley $
 */
...
function show_msg($error_no, $error_msg = 'ok', $success = 1, $quit = TRUE) {
		...
		echo '<input type="button" class="btn" onclick="history.back()" value="'.lang('click_to_back').'" /><br /><br /><br />';
...
<div class="main" style="margin-top:-145px;">
...
function createtable($sql, $dbver) {
	...
	$type = in_array($type, array('MYISAM', 'HEAP', 'MEMORY','InnoDB')) ? $type : 'InnoDB';
...
function show_header() {
	...
	global $step;
	$version = SHOPDZ_VERSION;
	$release = SHOPDZ_RELEASE;
...
<meta content="Comsenz-service Inc." name="Copyright" />
</head>
<div class="container">
	<div class="header">
		
		<span>ShopDZ $version $install_lang $release</span>
EOT;
...
function show_footer($quit = true) {
	...
	echo <<<EOT
		<div class="footer">&copy;2016 - 2026 <a href="http://www.comsenz-service.com/">Comsenz-service</a> Inc.</div>
	</div>
</div>
</body>
</html>
...
function check_env() {
	...
	if(!file_exists(ROOT_PATH.'./common/Conf/config.php')) {
		$errors[] = 'config_nonexistence';
		$quit = true;
	} elseif(!is_writeable(ROOT_PATH.'./common/Conf/config.php')) {
		$errors[] = 'config_unwriteable';
		$quit = true;
	}

	$checkdirarray = array(
		'attach' => ROOT_PATH.'./Attach',
	);
...
function check_adminuser($username, $password, $email) {
	...
	if($result['uid'] <= 0) {
		...
		$query = $db->query("INSERT INTO {$tablepre}admin (username,groupid,password,email,salt,ip,isfounder,lastdateline,dateline,avatar) VALUES ('$username', 0,'$password', '$email', '$salt', '$onlineip',1,0,'$timestamp','');");
		$uid = $db->insert_id($query);
	} else {
		$uid = $result['uid'];
		$email = $result['email'];
